...
if __name__ == '__main__':
  ...
  for link in links:
    ...
    ext = ".wmv"
    filename = re.search(r"[a-z]+[0-9]+[a-z]?/[0-9]+",video).group(0).replace("/","_")
    output_name = filename+ext
    # If there are more than one lecture a day, rename the output
    k = 2
    while len([i for i,v in enumerate(videos) if v[1] == output_name]):
      output_name = filename + '_' + str(k)+ext
      k += 1
